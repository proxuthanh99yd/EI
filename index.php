...
class AuthService
{
    ...
    public function updateUser($args)
    {
        ...
        foreach ($args as $key => $value) {
            if ($key == 'wp_user_gender' || $key == 'wp_user_birthday' || $key == 'wp_user_nationality') {
                update_field($key, $value, 'user_' . $user->ID);
            }
    ...
    public function getUserInfoById($id)
    {
        ...
        $user_info = [
            ...
            'nationality' => get_field('wp_user_nationality', 'user_' . $user->ID),
            'day' => '',
            'month' =>   '',
            'year' => '',
            'gender' => get_field('wp_user_gender', 'user_' . $user->ID),
            'phone' => get_user_meta($user->ID, 'billing_phone', true) ?: get_user_meta($user->ID, 'shipping_phone', true),
            'avatar' => get_field('wp_user_avatar', 'user_' . $user->ID),
        ];
        $wp_user_birthday =  get_field('wp_user_birthday', 'user_' . $user->ID);
        //   return $wp_user_birthday;
        if ($wp_user_birthday) {
            $date = \DateTime::createFromFormat('d/m/Y', $wp_user_birthday);
            $date->format('d/m/Y');
            // return $date;
